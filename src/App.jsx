...
function App({ onGoHome, onOpenSettings }) {
  ...
  function renderMobileFeed() {
    ...
    if (activeSectionId === "announcements") return <Announcements section={activeSection} onBack={goHome} isHost={isHost} onNavigate={navigateTo} mobileTab onOpenCreate={() => setFabOpen(true)} />;
    if (activeSectionId === "metrics")       return <MetricsContent />;
    if (activeSectionId === "rooms")         return <RoomsContent />;
    return null;
  }
        ...
        <>
          ...
          <AnimatePresence>
            {fabOpen && (
              ...
              >
                {[
                  { label: "Crear Post",       icon: FileText,  color: C.accent,    action: () => { setFabOpen(false); setShowNewPost(true); } },
                  { label: "Crear Difusión",   icon: Megaphone, color: C.orange,    action: () => { setFabOpen(false); setShowBroadcast(true); } },
                  { label: "Crear Story",      icon: Zap,       color: C.gold,      action: () => { setFabOpen(false); setShowNewStory(true); } },
                ].map((opt, i) => (
                  <motion.div
                    key={opt.label}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", stiffness: 380, damping: 38 }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                <div style={{ width: 32, height: 32, borderRadius: 10, background: `${C.accent}20`, border: `1px solid ${C.accent}35`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <FileText size={15} color={C.accentLight} />
                </div>
                <span style={{ fontFamily: font, fontSize: 16, fontWeight: 800, color: C.text }}>Crear Post</span>
              </div>
              <NewPostForm onSubmit={(text) => { setShowNewPost(false); navigateTo("recaps"); }} onClose={() => setShowNewPost(false)} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
