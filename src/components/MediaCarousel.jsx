...
import { ExternalLink, File as FileIcon, Link as LinkIcon, Play } from "lucide-react";
          ...
          >
            {current.type === "video" ? (
              ...
              <div style={{ position: "relative", width: "100%", height: "100%" }}>
                {/* Embeddable (YouTube/Drive) -> opens the fullscreen viewer
                    for in-app playback, same pattern as the "file" case
                    above. Everything else keeps opening the original URL
                    directly — unchanged from before. */}
                {current.embed ? (
                  <div
                    onClick={openGallery}
                    style={{ display: "block", width: "100%", height: "100%", position: "relative", cursor: onOpenGallery ? "pointer" : "default" }}
                  >
                    {current.thumb ? (
                      <>
                        <img
                    ...
                    {/* Play cue — signals in-app playback, matching the same
                        overlay LinkPreviewCard shows in the composer/inline
                    ...
                    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <div style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Play size={19} color="#fff" fill="#fff" style={{ marginLeft: 2 }} />
                      </div>
                    </div>
                  </div>
                ) : (
